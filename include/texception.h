...
#define TExceptionN(n)                                                         \
public:                                                                        \
  virtual const char *name() const { return #n; }
#define TExceptionM(m)                                                         \
public:                                                                        \
  virtual const char *message() const { return m; }
#define TExceptionM1(m, a)                                                     \
public:                                                                        \
...
#define TExceptionM3(m, a, b, c)                                               \
public:                                                                        \
  virtual const char *message() const {                                        \
    char *buf;                                                                 \
    asprintf(&buf, m, a, b, c);                                                \
    return buf;                                                                \
  }
#define TExceptionM4(m, a, b, c, d)                                            \
public:                                                                        \
  virtual const char *message() const {                                        \
    char *buf;                                                                 \
    asprintf(&buf, m, a, b, c, d);                                             \
    return buf;                                                                \
  }
...
class TZeroBasedIndexOutOfRangeException : public TIndexOutOfRangeException {
  ...
  TZeroBasedIndexOutOfRangeException(int index_, int total_num_)
      : TIndexOutOfRangeException(0, index_, total_num_ - 1) {}
};

class TErrnoException : public TException {
  TExceptionN(TErrnoException);
  TErrnoException(int error = -1) : err(error) {
    if (err < 0)
      err = errno;
  }
...
class TOperationErrnoException : public TErrnoException {
  ...
  TOperationErrnoException(const char *operation_, int error = -1)
      : TErrnoException(error), operation(operation_) {}
...
class TFileOperationErrnoException : public TErrnoException {
  ...
  TFileOperationErrnoException(const char *filename_, const char *operation_,
                               int err_ = -1)
      : TErrnoException(err_), filename(strdup(filename_)),
        operation(strdup(operation_)) {}
  //   virtual ~TFileOperationErrnoException() { free(filename);
  //   free(operation); }
  const char *filename;
  const char *operation;
  TExceptionM3("%s: %s (during %s)", filename, TErrnoException::message(),
               operation);
...
class TInternalErrorException : public TException {
  ...
  TInternalErrorException(const char *error_ = "unspecific error")
      : error(strdup(error_)) {}
...
class TNotInitializedException : public TInternalErrorException {
  ...
  TExceptionM1("internal_error: object of type '%s' is not initialized",
