...
private:
  ...
  public:
    ...
    char *data() {
      return (char *)(this +
                      1); // 'this + 1' means 'the byte following this object'
    }
    // character access
    ...
    // return pointer to the zero string representation (string conatining a
    // literal 0: "0" (and not "\0"))
    static Rep *zeroRep() {
      if (zero == 0)
        createZeroRep();
      return zero;
...
public:
  ...
  tstring scanToken(size_t &scanner, int flags, const char *allow = 0,
                    const char *forbid = 0, bool allow_quoted = false) const;
  /// scan a token or quoted string to out with scanner
  tstring scanString(size_t &scanner, int flags, const char *allow = 0,
                     const char *forbid = 0) const {
  ...
  tstring scanUpTo(size_t &scanner, char upto) const {
    int start(scanner);
    while ((scanner < rep->len) && ((*rep)[scanner] != upto))
      ++scanner;
  ...
  tstring scanRest(size_t &scanner) const {
    if (scanner < rep->len) {
      int start(scanner);
      scanner = rep->len;
      return substr(start, scanner);
  ...
  /**
   ...
   @param crop_space   true == remove leading/trailing spaces not protected by
   quotes
   */
  ...
  /// remove the first words that fit into a string of length max and return in
  /// block format
  ...
  /**@name search/replace */
  /*@{*/
  /// replace substring search with replace, return number of replacements (not
  /// regexp, use TRegEx to match regular expressions)
  int searchReplace(const tstring &search, const tstring &replace,
                    ...
                    const tstring &post_padstring = tstring(),
                    tvector<int> *match_pos = 0, int max_num = INT_MAX);
  /// return number of occurences of pat (not regexp) returns -1 on empty pat
  int search(const tstring &pat, bool ignore_case = false,
             bool whole_words = false, int progress = 0,
             tvector<int> *match_pos = 0) const; // throw(StringIsEmpty);
  /// replace substring
  ...
  void removeDirSlash();
  /// extract part after the last dot (empty string if no extension, leading dot
  /// is ignored)
...
public:
  ...
  /** detach from string pool and make sure at least minsize bytes of mem are
   available (use this before the dirty version sprintf to make it clean) (use
   this before the clean version sprintf to make it fast)
   */
  void detachResize(size_t minsize);
  /// detach from string pool and declare that string might be externally
  /// modified (the string has become vulnerable)
...
tvector<tstring> split(const tstring &s, const char *separator,
                       bool allow_quoting = false, bool crop_space = false);

/// join, reverse the effect of split
tstring join(const tvector<tstring> &a, const tstring &separator);

/// try to preserve case from 'from' to 'to' and return altered 'to' with case
/// from 'from'
...
/// Create progress bar
const char *progressBar(const char *message = 0, unsigned int n = 0,
                        unsigned int max = 0, int width = 79);
