...
public:
    ...
    ~TFile() {}
    void invalidateStat() const {
        const_cast<TFile*>(this)->stated = false;
    }
    ...
    mode_t protection() const {
        getstat();
        return statbuf.st_mode & (S_ISUID|S_ISGID|S_ISVTX|S_IRWXU|S_IRWXG|S_IRWXO);
    }
    bool isdir() const {
        getstat();
    ...
    bool devicetypeApplies() const {
        return ischardev() || isblockdev();
    }
    mode_t filetypebits() const {
        return statbuf.st_mode & S_IFMT;
    }
    TFileInstance instance() const {
        return TFileInstance(device(), inode());
    }
    FileType filetype() const;
    tstring filetypeLongStr() const;
    char filetypeChar() const;
    tstring filetypeStr7() const;
    static void followLinks(bool follow = true) {
        follow_links = follow;
    }
    static size_t numStated() {
        return num_stated;
    }
...
public:
    ...
    TDir(const tstring& fname, TSubTreeContext& subTreeContext_, size_t depth = 0): TFile(fname), scanned(false), subTreeContext(&subTreeContext_), depth(depth) {
        init();
    }
    TDir(const TFile& file_, TSubTreeContext& subTreeContext_, size_t depth = 0): TFile(file_), scanned(false), subTreeContext(&subTreeContext_), depth(depth) {
        init();
    }
    ~TDir() {}
    ...
    const TFile& file(const tstring& fname) const {
        scan();
        if(name2fileid.contains(fname)) return files[name2fileid[fname]];
        else throw TNotFoundException();
    }
    const TDir & dir (const tstring& fname) const {
        scan();
        if(name2dirid .contains(fname)) return dirs [name2dirid [fname]];
        else throw TNotFoundException();
    }
    const TFile& file(size_t index) const {
        scan();
        if(index >= files.size()) throw TZeroBasedIndexOutOfRangeException(index, files.size());
        return files[index];
    }
    const TDir & dir (size_t index) const {
        scan();
        if(index >= dirs .size()) throw TZeroBasedIndexOutOfRangeException(index, dirs .size());
        return dirs [index];
    }
    ...
    static void resetVerboseNum() {
        old_verbose_num = verbose_num = 0;
    };
...
private:
    ...
    void init() {
        ...
        if(subTreeContext->root == 0) subTreeContext->root = this;
    }
