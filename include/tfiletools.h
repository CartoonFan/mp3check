...
//
// 2007 24 Oct: removed __STRICT_ANSI__ support, fixed dev_t and made it more
// robust, fixed operator < for TFileInstance
...
public:
  ...
  const tstring &name() const {
    if (name_.empty())
      throw TNotInitializedException("TFile");
    return name_;
  }
  ...
  // stat fields
  mydev_t device() const {
    getstat();
    return dev_t2mydev_t(statbuf.st_dev);
  }
  ino_t inode() const {
    getstat();
    return statbuf.st_ino;
  }
  nlink_t hardlinks() const {
    getstat();
  ...
  mydev_t devicetype() const {
    getstat();
    return dev_t2mydev_t(statbuf.st_rdev);
  }
  off_t size() const {
    getstat();
...
public:
  TSubTreeContext(bool cross_filesystems, size_t max_depth = 0xffffffff)
      : root(0), cross_filesystems(cross_filesystems), max_depth(max_depth) {}
  TDir *root;
...
public:
  ...
  TDir(const tstring &fname, TSubTreeContext &subTreeContext_, size_t depth = 0)
      : TFile(fname), scanned(false), subTreeContext(&subTreeContext_),
        depth(depth) {
    init();
  }
  TDir(const TFile &file_, TSubTreeContext &subTreeContext_, size_t depth = 0)
      : TFile(file_), scanned(false), subTreeContext(&subTreeContext_),
        depth(depth) {
    init();
  }
  ~TDir() {}
  ...
  // read only interface
  const TFile &file(const tstring &fname) const {
    scan();
    if (name2fileid.contains(fname))
      return files[name2fileid[fname]];
    else
      throw TNotFoundException();
  }
  const TDir &dir(const tstring &fname) const {
    scan();
    if (name2dirid.contains(fname))
      return dirs[name2dirid[fname]];
    else
      throw TNotFoundException();
  }
  const TFile &file(size_t index) const {
    scan();
    if (index >= files.size())
      throw TZeroBasedIndexOutOfRangeException(index, files.size());
    return files[index];
  }
  const TDir &dir(size_t index) const {
    scan();
    if (index >= dirs.size())
      throw TZeroBasedIndexOutOfRangeException(index, dirs.size());
    return dirs[index];
  }
  ...
  bool containsFile(const tstring &fname) const {
    scan();
    return name2fileid.contains(fname);
  }
  bool containsDir(const tstring &fname) const {
    scan();
    return name2dirid.contains(fname);
  }
  bool contains(const tstring &fname) const {
    return containsDir(fname) || containsFile(fname);
  }
  bool isEmpty() const { return dirs.empty() && files.empty(); }
  void invalidateContents() { freeMem(); }
  size_t numRecursive(bool low_mem = true, const char *verbose = 0,
                      bool count_files = true, bool count_dirs = true) const;
  void freeMem() const {
    if (scanned) {
...
private:
  ...
  void init() {
    ...
    if (subTreeContext->root == 0)
      subTreeContext->root = this;
  }
...
tvector<tstring> filterExtensions(const tvector<tstring> &list,
                                  const tvector<tstring> &extensions,
                                  bool remove = false);
void makeDirectoriesIncludingParentsIfNecessary(const tstring &dirname,
                                                bool verbose = false,
                                                bool dummy = false);
