...
public:
  ...
  bool allMatchesSubstring(const char *str, tvector<tvector<int>> &all,
                           int eflags = 0, int progress = 0,
                           int progmode = 0) const;
  ...
  class TErroneousRegexException : public TException {
    ...
    TErroneousRegexException(int error_, const regex_t &preg_)
        : error(error_), preg(preg_) {}
  ...
  class TNoSubRegexException : public TException {
    TExceptionN(TErroneousRegexException);
  };
